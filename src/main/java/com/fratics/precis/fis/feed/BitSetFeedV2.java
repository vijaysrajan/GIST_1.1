...
public class BitSetFeedV2 extends PrecisProcessor {
    ...
    public boolean process(ValueObject o) throws Exception {
        ...
        StringBuilder tmpDimVal = new StringBuilder(2048);
        while ((str = ps.readStream()) != null) {
            ...
            for (int i = 0; i < fi.length; i++) {
                ...
                tmpDimVal.setLength(0);
                tmpDimVal.append(fi[i].getSchemaElement().fieldName);
                tmpDimVal.append(PrecisConfigProperties.OUTPUT_DIMVAL_SEPERATOR);
                tmpDimVal.append(str[fi[i].getSchemaElement().fieldIndex]);

                // Checks if the value is present in DimValindex.
                if (DimValIndex.dimMap.containsKey(tmpDim)) {
                    if (DimValIndex.dimValMap.containsKey(tmpDimVal.toString())) {
                        // If present, add them to a bit set object.
                        int index1 = DimValIndex.dimMap.get(tmpDim);
                        int index2 = DimValIndex.dimValMap.get(tmpDimVal.toString());
                        e.setBit(index1);
                        e.setBit(index2);
